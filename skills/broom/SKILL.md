...
| **修改监听地址（局域网访问）** | 在 `~/.config/broom/broom.yaml` 添加 `listen_addr: "0.0.0.0"`；或在 `config.BroomConfig` 新增字段后在 `cmd/broom/main.go` 传给 `proxy.Server`；默认为空/127.0.0.1 |
| **新增配置字段** | 1) `config/config.go` BroomConfig 添加字段；2) 在 `cmd/broom/main.go` 使用字段传给对应组件；3) 更新 `~/.config/broom/broom.yaml` |
